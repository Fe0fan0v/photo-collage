...
export class FinalScreen {
  ...
  render() {
    ...
    const telegramSection = createElement('div', { className: 'final-telegram-section' });

    // Promotion text
    const promoText = createElement('p', { className: 'final-telegram-text' });
    promoText.textContent = 'Выиграть тарелку из новой коллекции в Telegram';
    telegramSection.appendChild(promoText);

    // Telegram logo (using SVG icon)
    const telegramLogo = createElement('div', { className: 'final-telegram-logo' });
    telegramLogo.innerHTML = `
      <svg viewBox="0 0 240 240" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <linearGradient id="telegram-gradient-final" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" style="stop-color:#2AABEE;stop-opacity:1" />
            <stop offset="100%" style="stop-color:#229ED9;stop-opacity:1" />
          </linearGradient>
        </defs>
        <circle cx="120" cy="120" r="120" fill="url(#telegram-gradient-final)"/>
    ...
    // Make telegram logo clickable
    const telegramLink = createElement('a', {
      className: 'final-telegram-link',
      href: 'https://t.me/seletti_russia',
      target: '_blank',
      rel: 'noopener noreferrer'
    });
    telegramLink.appendChild(telegramLogo);
    telegramSection.appendChild(telegramLink);

    // "Хочу тарелку!" text
    const wantPlateText = createElement('a', {
      className: 'final-want-plate-text',
      href: 'https://t.me/seletti_russia',
      target: '_blank',
      rel: 'noopener noreferrer'
    });
    wantPlateText.textContent = 'Хочу тарелку!';
    telegramSection.appendChild(wantPlateText);

    scrollContent.appendChild(telegramSection);

    // Action buttons (same as success screen)
